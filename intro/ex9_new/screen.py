...
class Screen:
    ...
    def _bind_key(self, key, func, func2=None):
        """
        ...
        if key not in self._boundKeys:
            ...
            if func2 is not None:
                self._screen.onkeyrelease(func2, key)
            self._boundKeys.append(key)

    def _bind_keys(self):
        self._bind_key("Left", self._handle_left_press, self._handle_left_release)
        self._bind_key("Right", self._handle_right_press, self._handle_right_release)
        self._bind_key("Up", self._handle_up_press, self._handle_up_release)
        self._bind_key("space", self._handle_space_press, self._handle_space_release)
    ...
    def _handle_up_press(self):
        self._is_up_pressed = True
    def _handle_up_release(self):
        self._is_up_pressed = False
