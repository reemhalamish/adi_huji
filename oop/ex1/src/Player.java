...
public class Player {
	...
	//All following are for interactions with the human player.
	private static final String HUMAN_START_TURN = "Press 1 to display the board. Press 2 to make a move:";
	private static final String HUMAN_ASK_ROW = "Enter the row number:";
	private static final String HUMAN_ASK_START_STICK = "Enter the index of the leftmost stick:";
	private static final String HUMAN_ASK_END_STICK = "Enter the index of the rightmost stick:";
	private static final String HUMAN_ERROR = "Unsupported command";
	private static final int HUMAN_DISPLAY_BOARD = 1;
	private static final int HUMAN_MAKE_MOVE = 2;
	...
	private Move produceRandomMove(Board board){
		int randRow = chooseRandomRow(board);
		int randStart = chooseRandomStart(board, randRow);
		int randEnd = chooseRandomEnd(board, randRow, randStart);
		return new Move(randRow, randStart, randEnd);
	}

	/**
	 * chooses a random row that has at least one unmarked stick.
	 * @param board the current board in the game
	 * @return random row to choose sticks from
	 */
	private int chooseRandomRow(Board board) {
		Random random = new Random();
		while (true) {
			int randomRow = random.nextInt(board.getNumberOfRows()) + 1;
			for (int i = 1; i <= board.getRowLength(randomRow); i++)
	...
	/**
	 * chooses a random start stick for the sequence in a given row
	 * @param board the current board in the game
	 * @param row given legal row
	 * @return a random unmarked stick in given row
	 */
	private int chooseRandomStart(Board board, int row){
		Random random = new Random();
		while (true) {
			int randomStart = random.nextInt(board.getRowLength(row)) + 1;
			if (board.isStickUnmarked(row, randomStart)){
				return randomStart;
			}
		}
	...
	/**
	 * chooses a random end for the sequence.
	 * for given start stick, counts unmarked sticks ahead until reaching an unmarked stick,
	 * and chooses a random end stick.
	 * @param board current board in the game
	 * @param row given row to mark the sequence
	 * @param startStick given stick start
	 * @return random end stick for sequence
	 */
	private int chooseRandomEnd(Board board, int row, int startStick){
		Random random = new Random();
		int steps = 0;
		for(int i = startStick + 1; i <= board.getRowLength(row); i++){
		...
		return startStick + random.nextInt(steps + 1);
	}
	...
	private Move produceSmartMove(Board board) {
		Move move = getWinningMoveIfOnlyOneSeq(board);
		if (move != null) {
			return move;
		} else {
			return produceRandomMove(board);
	 ...
	 *
	 * @param board
	 * @return winning move if found one, else null
	 */
	private Move getWinningMoveIfOnlyOneSeq(Board board) {
		boolean foundSequence = false;
		int start = 0, end = 0, row = 0;
		for (int i = 1; i <= board.getNumberOfRows(); i++) {
			// first for, find "start"
			...
			// once found start, find end
			for (int j = start; j <= board.getRowLength(i); j++) {
				if(board.isStickUnmarked(i, j)){
					end = j;
				} else {
					break;
			...
			// last for to check its the only sequence
			for (int j = end + 1; j <= board.getRowLength(i); j++) {
				if (board.isStickUnmarked(i, j)) {
					return null;
				}
		...
		return new Move(row, start, end - 1);
	}
	...
	private Move produceHumanMove(Board board){
		while (true) {
			System.out.println(HUMAN_START_TURN);
			int userInput = scanner.nextInt();
			switch (userInput) {
				case HUMAN_DISPLAY_BOARD:
					System.out.println(board);
					break;
				case HUMAN_MAKE_MOVE:
					System.out.println(HUMAN_ASK_ROW);
					int userRow = scanner.nextInt();
					System.out.println(HUMAN_ASK_START_STICK);
					int userStart = scanner.nextInt();
					System.out.println(HUMAN_ASK_END_STICK);
					int userEnd = scanner.nextInt();
					return new Move(userRow, userStart, userEnd);
				default:
					System.out.println(HUMAN_ERROR);
			}
		}
	}
